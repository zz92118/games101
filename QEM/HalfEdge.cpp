...
Face* TriMesh::createFace(Vert* vertexs[3]) {
	...
	//建立一个三角形的半边
	for (int i = 0; i < 3; i++) {
		edges[i] = createEdge(vertexs[i % 3], vertexs[(i + 1) % 3]); //构造的edge是有方向的
		if (edges[i] == NULL) {
...
HalfEdge* TriMesh::createEdge(Vert* v1, Vert* v2) {
	//根据两个顶点创建half edge数据结构 有方向 从v1---v2
	// 是有向边
	//vert
	//opposite
	//face
	//edge-》next 在createface函数中
	if (v1 == NULL || v2 == NULL) {
		return NULL;
	}
...
//确定位于三角网格边界的点 包括点和边都有判断边界的isboundary变量
...
std::vector<Vert*> TriMesh::getNeighborVertexs(const Vert* vertex) {//访问权限冲突 nullptr
...
std::vector<HalfEdge*> TriMesh::getEdgesFromVertex(const Vert* vertex) {
	...
	HalfEdge* he = vertex->halfEdge;//从点出发的半边
	HalfEdge* temp = he;
	do {
		if (temp->isBoundary) {
...
std::vector<Face*> TriMesh::getFacesFromVertex(const Vert* vertex)//获得一个点的所有邻接face
{
...
std::vector<Vert*>TriMesh::getBoundaryNeighborVertexs(const Vert* vertex) {//获得边界
...
std::vector<HalfEdge*> TriMesh::getEdgesFromFace(const Face* face) { //获得面的所有半边
	std::vector<HalfEdge*> result;
	HalfEdge* he;
	he = face->halfEdge;
	result.push_back(he);
