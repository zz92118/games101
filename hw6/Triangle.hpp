...
class MeshTriangle : public Object//class 包括了triangle
...
{
    Intersection inter; // 初始化 intersaction的happened字段为false（一开始没有相交） 所以一开始return的都是false

    if (dotProduct(ray.direction, normal) > 0)
        return inter;
    double u, v, t_tmp = 0;
    //s1
    Vector3f pvec = crossProduct(ray.direction, e2);// e1 e2已经定义了
    ...
    // 构造结构体inter并返回 返回的是焦点的信息
    inter.happened = true;
    inter.coords = Vector3f(ray(t_tmp));//焦点位置 Ray.hpp 返回的重载运算符 operator（） 返回焦点位置
    inter.normal = normal;//三角形的法线
    inter.distance = t_tmp;// 没有速度 用t来代替intersection
    inter.obj = this;//相交的物体是这个三角形 Triangle 继承Object
    inter.m = m;
    return inter;
}
